...
public class RateLimterController2 {
    ...
    private String createConfig(@RequestBody RateLimitConfigDto2 rateLimitConfigDto2) {
        if (rateLimitConfigDto2!=null){
            ...
            RateLimitConfig rateLimitAppConfig = new RateLimitConfig(appId,null,null,null,rateLimitConfigDto2.getAppBucketConfig().getTime(),rateLimitConfigDto2.getAppBucketConfig().getUnit(),rateLimitConfigDto2.getAppBucketConfig().getNoOfAllowedHits(),rateLimitConfigDto2.getAppBucketConfig().getRateLimitLevel());
            rateLimitRepo.save(rateLimitAppConfig);

            RateLimitConfig rateLimitServiceConfig = new RateLimitConfig(appId+"~"+serviceName,null,null,null,rateLimitConfigDto2.getServiceBucketConfig().getTime(),rateLimitConfigDto2.getServiceBucketConfig().getUnit(),rateLimitConfigDto2.getServiceBucketConfig().getNoOfAllowedHits(),rateLimitConfigDto2.getServiceBucketConfig().getRateLimitLevel());
